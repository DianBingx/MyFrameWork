...
public class HotUpdateManager 
{
    ...
    static IEnumerator CheckVersion()
    {
        ...
        //取得服务器版本文件
        UnityWebRequest www = new UnityWebRequest(s_versionFileDownLoadPath);
        ...
        while (!www.isDone)
        {
            UpdateDateCallBack(HotUpdateStatusEnum.DownLoadingVersionFile, GetHotUpdateProgress(false, false, www.downloadProgress));
        ...
        s_versionFileCache = bundle.LoadAsset<TextAsset>(c_versionFileName).text;
        s_versionByteCache = www.downloadHandler.data;
        bundle.Unload(true);
    ...
    static IEnumerator DownLoadFile()
    {
        ...
        //取得服务器版本文件
        UnityWebRequest www = new UnityWebRequest(s_ManifestFileDownLoadPath);
        Debug.Log("服务器获取清单文件 ：" + s_ManifestFileDownLoadPath);
        //yield return www;

        while (!www.isDone)
        {
            UpdateDateCallBack(HotUpdateStatusEnum.DownLoadingManifestFile, GetHotUpdateProgress(true, false, www.downloadProgress));
            yield return new WaitForEndOfFrame();
        } 
        ...
        s_ManifestCache = bundle.LoadAsset<AssetBundleManifest>("AssetBundleManifest");
        s_ManifestByteCache = www.downloadHandler.data;
        bundle.Unload(false);
    ...
    static IEnumerator StartDownLoad()
    {
        ...
        for (int i = 0; i < s_downLoadList.Count; i++)
        {
            ...
            else
            {
                ...
                UnityWebRequest www = new UnityWebRequest(downloadPath);
                yield return www;
                ...
                else
                {
                    ...
                    ResourceIOTool.CreateFile(PathTool.GetAssetsBundlePersistentPath() + "/" + s_downLoadList[i].name , www.downloadHandler.data);
